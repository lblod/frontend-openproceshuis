...
export default class InventoryUpsertProcessModal extends Component {
  @service store;
  @service toaster;
  ...
  loadOptionsTask = keepLatestTask(async () => {
  ...
  @action
  async saveProcess() {
    if (this.isCreateNewProcess) {
      try {
        const number = await this.getNewProcessNumber();
        const now = new Date();
        const process = this.store.createRecord('conceptual-process', {
          title: this.cleanTitle,
          number,
          processGroups: [this.processGroup],
          created: now,
          modified: now,
          status: null,
        });
        await process.save();
        this.toaster.success('Proces succesvol aangemaakt', undefined, {
          timeOut: 5000,
        });
      } catch (error) {
        this.toaster.error(
          'Er liep iets mis bij het aanmaken van een nieuwe proces',
          undefined,
          {
            timeOut: 5000,
          },
      ...
      try {
        this.args.process.title = this.cleanTitle;
        this.args.process.processGroups = [this.processGroup];
        await this.args.process.save();
        this.toaster.success('Proces succesvol aangepast', undefined, {
          timeOut: 5000,
        });
      } catch (error) {
        this.toaster.error(
    ...
    this.args?.onProcessSaved?.();
  }

  async getNewProcessNumber() {
    try {
      const processes = await this.store.query('conceptual-process', {
        sort: '-number',
        page: { size: 1 },
      });
      const lastNumber = processes[0]?.number;
